# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v4.1 - Operator UI / Production-Ready Layout
...
# GUI class
# =========================================================
class KD240HeaterGuiV41:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v4.1")
    ...
    def make_entry(self, parent, label_text, default):
        ...
        entry.pack(fill="x", pady=(3, 8), ipady=7)
        return entry
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v4.1",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v4.1 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        self.lbl_tune_error = self.make_label_value(gain_body, "Tune Error", 0, 3, width=12,
                                                    value_font_size=12, label_font_size=8, value_pady=7)
        self.lbl_ktl = self.make_label_value(gain_body, "Model K / L / T", 0, 4, width=28,
                                             value_font_size=12, label_font_size=8, value_pady=7)
        self.lbl_tuned_gain = self.make_label_value(gain_body, "Tuned Gain", 0, 5, width=30,
                                                    value_font_size=12, label_font_size=8, value_pady=7)

        # Graph card
        plot_card = self.make_card(left, "Live Trend v4.1")
        ...
        self.btn_reset = self.make_button(cmd_grid, "Fault Reset", self.reset_heater, self.purple, width=10)
        ...
        tk.Checkbutton(
            tune_body,
            text="Auto copy tuned gain && RUN after DONE",
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v4_0_log_{}.csv".format(now_timestamp())
        if auto:
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v4_0_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
    ...
    def analyze(self, auto=False):
        """
        Operator-facing trend summary.

        This function intentionally does not calculate an arbitrary score.
        For heater control, the important information is:
          - whether STABLE was reached
          - how long it took to reach STABLE
        ...
        stable_time = self.stable_elapsed_sec
        if stable_time is None and stable_reached:
            for r in self.log_rows:
                if r["state"] == 2:
        ...
        final_error_ok = final_error_avg <= 1.0
        fault_ok = not fault_occurred
        stable_ok = stable_reached

        summary_ok = overshoot_ok and final_error_ok and fault_ok and stable_ok
        summary_text = "OK" if summary_ok else "CHECK"

        if stable_time is None:
            stable_text = "Not reached"
        else:
            stable_text = "{:.3f} s".format(stable_time)

        if settling_time is None:
            settling_text = "Not settled within ±1°C until end"
        ...
        metrics = (
            ...
            target, kp, ki,
            "Yes" if stable_reached else "No",
            stable_text,
            settling_text,
            max_temp,
            overshoot, "OK" if overshoot_ok else "CHECK",
            final_error_avg, "OK" if final_error_ok else "CHECK",
            oscillation_count,
            saturation_ratio * 100.0,
            "Yes" if fault_occurred else "No", "OK" if fault_ok else "CHECK",
            summary_text
        )

        if not auto:
            messagebox.showinfo("Trend Summary", metrics)

        self.log("")
        self.log("===== Trend Summary =====")
        for line in metrics.splitlines():
            self.log(line)

        self.add_result_history(kp, ki, overshoot, settling_for_table, final_error_avg, summary_text)
