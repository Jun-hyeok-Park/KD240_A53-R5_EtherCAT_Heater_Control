# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v3.2 - Tuned Gain TxPDO
...
class KD240HeaterGuiV3:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v3.2")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v3.2",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v3.2 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        # Moved to left/main area so tuned gain values are always visible.
        gain_card = self.make_card(left, "Gain / Auto Tune Status / Tuned Gain")
        gain_card.pack(fill="x", pady=(0, 12))

        gain_body = tk.Frame(gain_card, bg=self.panel)
        gain_body.pack(fill="x", padx=16, pady=(8, 14))
        ...
        self.result_badge.pack(side="left", fill="y", padx=(0, 12))
        ...
        self.lbl_metrics.pack(side="left", fill="both", expand=True)

        gain_action = tk.Frame(gain_body, bg=self.panel)
        gain_action.pack(side="right", fill="y", padx=(12, 0))

        self.btn_save_csv = self.make_small_button(gain_action, "Save CSV", self.save_csv, self.dark_button, width=11)
        self.btn_save_png = self.make_small_button(gain_action, "Save PNG", self.save_png, self.dark_button, width=11)
        self.btn_save_csv.pack(fill="x", pady=(0, 6))
        self.btn_save_png.pack(fill="x", pady=(0, 0))

        # Graph card
        plot_card = self.make_card(left, "Live Trend v3.2")
    ...
    def update_gain_status_view(self, status=None):
        """
        Show current manual entry values, last sent RxPDO values,
        and actual tuned gain values received through the expanded 48-byte TxPDO.
        """
        ...
        if status is not None:
            ...
            auto_error = str(status.get("auto_tune_error", 0))
            valid_text = "1" if status.get("tuned_gain_valid_flag", False) else "0"
        ...
        text = (
            "Entry       Target={} °C | Kp={} | Ki={}\n"
            "Last Sent   CW=0x{:04X} | Target={:.3f} °C | Kp={:.6f} | Ki={:.8f}\n"
            "Status      Heater={} | Auto={} | Source={}\n"
            "Tune Model  Valid={} | Error={} | K={} | L={} s | T={} s\n"
            "Tuned Gain  Kp={} | Ki={}"
        ).format(
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v3_1_log_{}.csv".format(now_timestamp())
        if auto:
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                "tune_k", "tune_l", "tune_t", "tune_kp", "tune_ki", "tuned_gain_valid", "auto_tune_error"
            ])
            for r in self.log_rows:
                writer.writerow([
                    ...
                    r.get("tune_k"), r.get("tune_l"), r.get("tune_t"), r.get("tune_kp"), r.get("tune_ki"), r.get("tuned_gain_valid", 0), r.get("auto_tune_error", 0)
                ])
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v3_1_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
