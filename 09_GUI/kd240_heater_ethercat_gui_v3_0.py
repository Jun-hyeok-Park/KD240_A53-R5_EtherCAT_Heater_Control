# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v3.0 - Gain Status Update
...
- Show current limitation: tuned Kp/Ki is not available through the current 20-byte TxPDO
...
def parse_txpdo(data):
    ...
    raw = bytes(data)
    status_word, state_packed, current_raw, error_raw, u_raw, duty = struct.unpack("<HHIIII", raw)
...
# GUI class
# =========================================================
class KD240HeaterGuiV3:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v3.0")
        ...
        self.tuned_gain_note = "현재 20-byte TxPDO에는 tune_kp/tune_ki 값이 포함되어 있지 않습니다."
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v3.0",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 20 byte | RUN/STOP | Auto Tune v3.0 | Optional CSV/PNG autosave",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        self.lbl_state = self.make_label_value(status_body, "Heater State", 0, 0, width=15)
        self.lbl_auto_state = self.make_label_value(status_body, "Auto Tune", 0, 1, width=15)
        self.lbl_status_word = self.make_label_value(status_body, "StatusWord", 0, 2, width=15)
        self.lbl_current_t = self.make_label_value(status_body, "Current Temp", 0, 3, width=15)
        self.lbl_error = self.make_label_value(status_body, "Error", 0, 4, width=15)
        self.lbl_u_ctrl = self.make_label_value(status_body, "Controller Output", 0, 5, width=15)
        self.lbl_duty = self.make_label_value(status_body, "Duty", 0, 6, width=15)
        ...
        # Graph card
        plot_card = self.make_card(left, "Live Trend v3.0")
        ...
        # Gain / Auto Tune status card
        gain_card = self.make_card(right, "Gain / Auto Tune Status")
        gain_card.pack(fill="x", pady=(0, 12))

        gain_body = tk.Frame(gain_card, bg=self.panel)
        gain_body.pack(fill="x", padx=16, pady=(8, 16))
        ...
        action_row = tk.Frame(gain_body, bg=self.panel)
        ...
        # PDO note card
        note_card = self.make_card(right, "PDO / Gain Note")
        note_card.pack(fill="both", expand=True)

        note_body = tk.Frame(note_card, bg=self.panel)
        note_body.pack(fill="both", expand=True, padx=16, pady=(8, 16))

        self.lbl_pdo_note = tk.Label(
            note_body,
            text=(
                "현재 TxPDO 20 byte 구성에는\n"
                "StatusWord / State / Temp / Error / Controller Output / Duty만 포함됩니다.\n\n"
                "따라서 Auto Tune 결과 Kp/Ki 실제 숫자는\n"
                "현재 GUI가 직접 읽을 수 없습니다.\n\n"
                "실제 tuned Kp/Ki까지 표시하려면\n"
                "A53 TxPDO에 tune_kp, tune_ki 필드를 추가해야 합니다."
            ),
            bg=self.panel,
            fg=self.subtext,
            justify="left",
        ...
        self.lbl_pdo_note.pack(fill="both", expand=True)
    ...
    def update_gain_status_view(self, status=None):
        """
        Show parameter/gain information that is actually available to the PC.

        Important limitation:
        Current TxPDO is 20 bytes and does not include tuned Kp/Ki result fields.
        The GUI can show the last manual gain sent through RxPDO and whether
        Apply Tuned Gain command was sent, but it cannot show the numeric tuned gain
        until the firmware exposes tune_kp/tune_ki through TxPDO.
        """
        ...
        try:
            entry_target = float(self.entry_target.get())
        ...
        if status is not None:
            heater_state = "{}({})".format(status.get("state_name", "UNKNOWN"), status.get("state", 0))
            auto_state = "{}({})".format(status.get("auto_tune_state_name", "UNKNOWN"), status.get("auto_tune_state", 0))

        if self.active_gain_source.startswith("Auto"):
            badge_color = self.purple
            badge_text = "AUTO GAIN"
        elif self.active_gain_source.startswith("Manual"):
            badge_color = self.blue
            badge_text = "MANUAL GAIN"
        elif self.active_gain_source.startswith("Stopped"):
            badge_color = self.dark_button
            badge_text = "STOPPED"
        else:
            badge_color = self.yellow
            badge_text = "GAIN STATUS"

        self.result_badge.config(text=badge_text, bg=badge_color, fg="white")
        ...
        text = (
            ...
            "  Source : {}\n\n"
            "Tuned Gain Value\n"
            "  Kp/Ki  : N/A in current 20-byte TxPDO"
        ).format(
            fmt_f(entry_target, 3),
            fmt_f(entry_kp, 6),
            fmt_f(entry_ki, 8),
            self.last_sent_control_word,
            self.last_sent_target,
            self.last_sent_kp,
            self.last_sent_ki,
            heater_state,
            auto_state,
            self.active_gain_source,
        )
    ...
    def connect(self):
        ...
        try:
            ...
            ret = self.wmx.SetDeviceName("KD240HeaterGuiV3")
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v3_0_log_{}.csv".format(now_timestamp())
        if auto:
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                "state", "state_name", "auto_tune_state", "auto_tune_state_name", "state_packed", "status_word", "entry_kp", "entry_ki", "gain_source"
            ])
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v3_0_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
