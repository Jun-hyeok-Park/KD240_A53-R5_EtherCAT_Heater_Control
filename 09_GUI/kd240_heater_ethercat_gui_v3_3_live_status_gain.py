# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v3.3 - Live Status Tuned Gain
...
class KD240HeaterGuiV3:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v3.3")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v3.3",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v3.3 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        # Tuned gain status is integrated into Live Status so the graph keeps its full height.
        self.lbl_gain_source = self.make_label_value(status_body, "Gain Source", 1, 0, width=15)
        self.lbl_entry_gain = self.make_label_value(status_body, "Entry Kp / Ki", 1, 1, width=15)
        self.lbl_last_sent = self.make_label_value(status_body, "Last Sent", 1, 2, width=15)
        self.lbl_tune_valid = self.make_label_value(status_body, "Tune Valid", 1, 3, width=15)
        self.lbl_tune_model = self.make_label_value(status_body, "K / L / T", 1, 4, width=15)
        self.lbl_tuned_kp = self.make_label_value(status_body, "Tuned Kp", 1, 5, width=15)
        self.lbl_tuned_ki = self.make_label_value(status_body, "Tuned Ki", 1, 6, width=15)
        ...
        # Graph card
        plot_card = self.make_card(left, "Live Trend v3.3")
    ...
    def update_gain_status_view(self, status=None):
        ...
        # Compact values so the Live Status card remains readable.
        self.lbl_gain_source.config(text=gain_source, bg=gain_color, fg="white" if gain_color != self.yellow else "#111827")
        self.lbl_entry_gain.config(text="{} / {}".format(fmt_f(entry_kp, 5), fmt_f(entry_ki, 6)))
        self.lbl_last_sent.config(text="CW=0x{:04X}".format(self.last_sent_control_word))
        self.lbl_tune_valid.config(text="{} / Err {}".format(tune_valid, tune_error))
        self.lbl_tune_model.config(text="{} / {} / {}".format(fmt_f(tune_k, 3), fmt_f(tune_l, 2), fmt_f(tune_t, 2)))
        self.lbl_tuned_kp.config(text=fmt_f(tune_kp, 6))
        self.lbl_tuned_ki.config(text=fmt_f(tune_ki, 8))
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v3_3_log_{}.csv".format(now_timestamp())
        if auto:
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v3_3_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
