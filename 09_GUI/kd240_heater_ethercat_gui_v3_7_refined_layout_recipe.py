# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v3.7 - Refined Layout / Recipe Gain Handling
...
class KD240HeaterGuiV36:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v3.7")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v3.7",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v3.7 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        # State summary is displayed inside the Live Status card in v3.7
        # to free the Connection / Control row and keep status text readable.
        ...
        for c in range(7):
            status_body.grid_columnconfigure(c, weight=1, uniform="status_top")

        # Status summary box replaces the previous Heater State / Auto Tune / StatusWord cells.
        # It spans the first three columns so the Korean/English status text is not clipped.
        ...
        self.state_box.grid(row=0, column=0, columnspan=3, sticky="nsew", padx=5, pady=5)
        ...
        self.state_badge.pack(side="left", padx=(8, 8), pady=8)
        ...
        self.state_desc.pack(side="left", fill="x", expand=True, padx=(0, 8), pady=8)
        ...
        self.lbl_stable_time.pack(side="right", padx=(0, 8), pady=8)
        ...
        for c in range(4):
            gain_body.grid_columnconfigure(c, weight=1, uniform="gain_row")

        # Equal-width gain information cells.
        self.lbl_gain_source = self.make_label_value(gain_body, "Gain Source", 0, 0, width=24,
                                                     value_font_size=13, label_font_size=8, value_pady=7)
        self.lbl_entry_gain = self.make_label_value(gain_body, "Entry / Last Sent", 0, 1, width=24,
                                                    value_font_size=10, label_font_size=8, value_pady=7)
        self.lbl_tune_model = self.make_label_value(gain_body, "Auto Tune Model", 0, 2, width=24,
                                                    value_font_size=10, label_font_size=8, value_pady=7)
        self.lbl_tuned_gain = self.make_label_value(gain_body, "Tuned Gain", 0, 3, width=24,
                                                    value_font_size=10, label_font_size=8, value_pady=7)

        # Graph card
        plot_card = self.make_card(left, "Live Trend v3.7")
        ...
        self.btn_apply_best = self.make_small_button(future_box, "Use Tuned Gain", self.use_tuned_gain, self.purple, width=15)
        ...
        self.btn_apply_recipe = self.make_small_button(recipe_box, "Use Recipe Gain", self.use_recipe_gain, self.cyan, width=16)
        ...
        tk.Checkbutton(
            form,
            text="Auto use tuned gain && RUN after DONE",
    ...
    def use_tuned_gain(self):
        tuned = self.get_valid_tuned_gain()
        if tuned is None:
            messagebox.showwarning("Use Tuned Gain", "아직 유효한 tuned Kp/Ki 값이 없습니다. Auto Tune DONE 후 다시 시도하세요.")
        ...
        self.update_gain_status_view()
        self.log("[AUTO] Tuned gain copied to Manual Control: Kp={:.6f}, Ki={:.8f}".format(tuned_kp, tuned_ki))
        self.log("[AUTO] Press RUN or Write Params to send the tuned parameters through RxPDO.")
        return True
    ...
    def use_recipe_gain(self):
        if self.loaded_recipe is None:
            messagebox.showinfo("Use Recipe Gain", "먼저 Load Recipe를 수행하세요.")
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v3_7_log_{}.csv".format(now_timestamp())
        if auto:
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v3_7_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
