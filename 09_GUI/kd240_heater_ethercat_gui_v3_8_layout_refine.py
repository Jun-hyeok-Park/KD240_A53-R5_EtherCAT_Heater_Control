# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v3.8 - Refined Layout / Recipe Gain Handling
...
class KD240HeaterGuiV36:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v3.8")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v3.8",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v3.8 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        # The global connection status badge is placed inside the Connection card.
        ...
        self.connection_badge.pack(side="right", padx=(8, 0))
        ...
        for c, wt in enumerate([36, 10, 10, 11, 10]):
        ...
        for c, wt in enumerate([16, 19, 9, 9, 23, 24]):
            gain_body.grid_columnconfigure(c, weight=wt)

        self.lbl_gain_source = self.make_label_value(gain_body, "Gain Source", 0, 0, width=16,
                                                     value_font_size=12, label_font_size=8, value_pady=7)
        self.lbl_entry_gain = self.make_label_value(gain_body, "Entry / Last Sent", 0, 1, width=22,
                                                    value_font_size=10, label_font_size=8, value_pady=7)
        self.lbl_tune_valid = self.make_label_value(gain_body, "Valid", 0, 2, width=8,
                                                    value_font_size=12, label_font_size=8, value_pady=7)
        self.lbl_tune_error = self.make_label_value(gain_body, "Err", 0, 3, width=8,
                                                    value_font_size=12, label_font_size=8, value_pady=7)
        self.lbl_ktl = self.make_label_value(gain_body, "K / L / T", 0, 4, width=28,
                                             value_font_size=10, label_font_size=8, value_pady=7)
        self.lbl_tuned_gain = self.make_label_value(gain_body, "Tuned Gain", 0, 5, width=30,
                                                    value_font_size=10, label_font_size=8, value_pady=7)

        # Graph card
        plot_card = self.make_card(left, "Live Trend v3.8")
        ...
        self.btn_save_recipe = self.make_small_button(recipe_box, "Save Recipe", self.save_recipe, self.dark_button, width=14)
        self.btn_load_recipe = self.make_small_button(recipe_box, "Load Recipe", self.load_recipe, self.dark_button, width=14)

        self.btn_save_recipe.grid(row=0, column=0, sticky="ew", padx=(0, 4), pady=4)
        self.btn_load_recipe.grid(row=0, column=1, sticky="ew", padx=(4, 0), pady=4)
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v3_8_log_{}.csv".format(now_timestamp())
        if auto:
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v3_8_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
