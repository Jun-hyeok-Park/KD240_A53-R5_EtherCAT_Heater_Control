# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v3.9 - Operator UI / Production-Ready Layout
...
class KD240HeaterGuiV36:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v3.9")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v3.9",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v3.9 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        self.connection_badge.pack(side="left", padx=(18, 0))
        ...
        # Graph card
        plot_card = self.make_card(left, "Live Trend v3.9")
        ...
        self.btn_write = self.make_button(pi_body, "Write Gain Only", self.write_params, self.blue, width=28)
        self.btn_clear = self.make_button(pi_body, "Clear Command", self.clear_control, self.dark_button, width=28)

        self.btn_write.pack(fill="x", pady=(8, 3))
        self.btn_clear.pack(fill="x", pady=3)

        tk.Label(
            pi_body,
            text="RUN sends Target/Kp/Ki together. Write Gain Only is for communication check.",
            bg=self.panel,
            fg=self.muted,
            font=("Segoe UI", 8),
            anchor="w",
        ...
        self.btn_apply_best = self.make_button(tune_grid, "Load Tuned Gain", self.use_tuned_gain, self.purple, width=14)
        ...
        tk.Checkbutton(
            tune_body,
            text="Auto load tuned gain && RUN after DONE",
    ...
    def write_params(self):
        if self.write_output(self.last_control_word, log_tx=True):
            self.log("[OK] Gain write requested with current command=0x{:04X}".format(self.last_control_word))
    ...
    def use_tuned_gain(self):
        tuned = self.get_valid_tuned_gain()
        if tuned is None:
            messagebox.showwarning("Load Tuned Gain", "아직 유효한 tuned Kp/Ki 값이 없습니다. Auto Tune DONE 후 다시 시도하세요.")
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v3_9_log_{}.csv".format(now_timestamp())
        if auto:
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v3_9_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
