# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v2.0

v2.0 scope
- Keep stable Tkinter + WMX3 structure
- Split graph into Temperature / Controller Output
- Add graph visibility checkboxes
- Add rolling window selector
- Add settling band display
- Add result history table
- Add Auto Tuning placeholder button for later Ziegler-Nichols implementation
...
class KD240HeaterGuiV2:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v2.0")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v2.0",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 20 byte | Split Trend Graph | Auto Tuning placeholder",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        # Graph card
        plot_card = self.make_card(left, "Live Trend v2.0")
        ...
        self.make_check(plot_option, "U Ctrl", self.var_show_u).pack(side="left", padx=8)
        ...
        # Log card below graph, left side
        ...
        self.entry_duration = self.make_entry(form, "Auto Test Duration [s]", str(DEFAULT_DURATION))
        ...
        # Auto tuning placeholder card
        tune_card = self.make_card(right, "Auto Tuning / v3.0 Placeholder")
        tune_card.pack(fill="x", pady=(0, 12))

        tune_body = tk.Frame(tune_card, bg=self.panel)
        tune_body.pack(fill="x", padx=16, pady=(6, 16))

        tune_info = (
            "Ziegler-Nichols 기반 Auto Tuning은\n"
            "다음 단계에서 구현 예정입니다.\n"
            "현재 v2.0에서는 버튼과 UI 자리만 준비합니다."
        )
        tk.Label(
            tune_body,
            text=tune_info,
        ...
        self.btn_auto_tune = self.make_button(tune_body, "Start Z-N Auto Tuning", self.auto_tuning_placeholder, self.yellow, width=28)
        self.btn_auto_tune.pack(fill="x", pady=4)

        self.btn_apply_best = self.make_button(tune_body, "Apply Recommended Gain", self.apply_recommended_placeholder, self.dark_button, width=28)
        self.btn_apply_best.pack(fill="x", pady=4)
    ...
    def configure_axes(self):
        ...
        self.ax_out.set_ylabel("Output [%]", fontsize=9)
        self.ax_out.set_xlabel("Time [s]", fontsize=9)
        self.ax_temp.set_title("Temperature Response", fontsize=10, fontweight="bold")
        self.ax_out.set_title("Controller Output", fontsize=10, fontweight="bold")
    ...
    def stop_auto_test(self):
        self.stop_auto_timer()
        self.logging_enabled = False
        self.add_event_marker("AUTO_STOP")
    ...
    def update_plot(self):
        ...
        if self.var_show_u.get():
            self.ax_out.plot(times, us, label="U Ctrl [%]", color="#7C3AED", linewidth=1.8)
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v2_log_{}.csv".format(now_timestamp())
        if auto:
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                "u_ctrl", "u_percent", "duty_cnt", "duty_percent",
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v2_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
