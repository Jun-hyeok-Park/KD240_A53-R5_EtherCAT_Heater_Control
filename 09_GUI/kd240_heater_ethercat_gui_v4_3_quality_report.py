# -*- coding: utf-8 -*-
"""
KD240 EtherCAT Virtual Heater PI Controller GUI v4.3 - Operator UI / Quality Report
...
# GUI class
# =========================================================
class KD240HeaterGuiV43:
    def __init__(self, root):
        self.root = root
        self.root.title("KD240 EtherCAT Virtual Heater PI Controller v4.3")
    ...
    def build_ui(self):
        ...
        title = tk.Label(
            header,
            text="KD240 EtherCAT Virtual Heater PI Controller v4.3",
            bg=self.bg,
            fg=self.text,
        ...
        subtitle = tk.Label(
            self.root,
            text="WMX I/O Space | RxPDO 14 byte / TxPDO 48 byte | RUN/STOP | Auto Tune v4.3 | Tuned Gain Display",
            bg=self.bg,
            fg=self.subtext,
            font=("Segoe UI", 10)
        )
        ...
        # Graph card
        plot_card = self.make_card(left, "Live Trend v4.3")
    ...
    def connect(self):
        ...
        try:
            ...
            ret = self.wmx.SetDeviceName("KD240HeaterGuiV43")
    ...
    def save_csv(self, auto=False):
        ...
        default_name = "kd240_heater_v4_3_log_{}.csv".format(now_timestamp())
        if auto:
    ...
    def save_png(self, auto=False):
        ...
        default_name = "kd240_heater_v4_3_result_{}.png".format(now_timestamp())
        if auto:
            filename = default_name
        else:
            filename = filedialog.asksaveasfilename(
    ...
    def make_report_card(self, parent, title, value, result_text, ok):
    ...
    def show_analysis_report_window(self):
        ...
        def add_field(col, label, var, width=8):
            ...
            box.grid(row=1, column=col, sticky="ew", padx=6, pady=(0, 12))
            ...
            ent.pack(fill="x", ipady=4, pady=(3, 0))
            return ent

        mode_box = tk.Frame(criteria, bg=self.panel)
        mode_box.grid(row=1, column=0, sticky="ew", padx=(12, 6), pady=(0, 12))
        ...
        def add_table_row(row_idx, metric, value, criteria_text, result_text, ok):
            bg = self.panel2 if row_idx % 2 == 0 else self.panel
            color = self.green if ok else self.red
            ...
            for col, txt in enumerate(cells):
                lbl = tk.Label(
                    rows_frame,
                    text=txt,
                    bg=color if col == 3 else bg,
                    fg="white" if col == 3 else self.text,
                    font=("Segoe UI", 9, "bold") if col == 3 else ("Segoe UI", 9),
