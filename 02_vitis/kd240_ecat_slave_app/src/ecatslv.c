...
TSYNCMAN ESCMEM * GetSyncMan(UINT8 channel)
{
    ...
    UINT8  sm_stat = 0U;
    UINT8  sm_enable = 0U;
    UINT8  sm_pdi_ctrl = 0U;

    base = (UINT16)(ESC_SYNCMAN_REG_OFFSET + (channel * SIZEOF_SM_REGISTER));

    HW_EscReadWord(sm_addr,     (UINT16)(base + 0U));
    HW_EscReadWord(sm_len,      (UINT16)(base + 2U));
    HW_EscReadByte(sm_ctrl,     (UINT16)(base + 4U));
    HW_EscReadByte(sm_stat,     (UINT16)(base + 5U));
    HW_EscReadByte(sm_enable,   (UINT16)(base + 6U));
    HW_EscReadByte(sm_pdi_ctrl, (UINT16)(base + 7U));

    SyncManInfo.AddressLength =
        (((UINT32)sm_len << SM_LENGTH_SHIFT) & SM_LENGTH_MASK) |
        ((UINT32)sm_addr & SM_ADDRESS_MASK);

    /*
     * Settings[0] is used by SSC as a 32-bit packed value.
     *
     * bit  0~7  : Control
     * bit  8~15 : Status
     * bit 16~23 : Activate / Enable
     * bit 24~31 : PDI Control
     */
    SyncManInfo.Settings[0] =
        ((UINT32)sm_ctrl) |
        ((UINT32)sm_stat << 8) |
        ((UINT32)sm_enable << 16) |
        ((UINT32)sm_pdi_ctrl << 24);
...
UINT8 CheckSmSettings(UINT8 maxChannel)
{
    ...
    UINT16 SMLength = 0U;
    UINT16 SMAddress = 0U;
    UINT32 SMSettings = 0U;
    UINT32 SMActivate = 0U;
    ...
    if ((nMaxEscAddress < MAX_PD_WRITE_ADDRESS)
        ...
        || (nMaxEscAddress < MAX_MBX_READ_ADDRESS))
    {
        DBG_APP("CHKSM FAIL: invalid firmware range");
        DBG_APP("nMaxEscAddress       = 0x%04x", (unsigned int)nMaxEscAddress);
        DBG_APP("MAX_PD_WRITE_ADDRESS = 0x%04x", (unsigned int)MAX_PD_WRITE_ADDRESS);
        DBG_APP("MAX_PD_READ_ADDRESS  = 0x%04x", (unsigned int)MAX_PD_READ_ADDRESS);
        DBG_APP("MAX_MBX_WRITE_ADDR   = 0x%04x", (unsigned int)MAX_MBX_WRITE_ADDRESS);
        DBG_APP("MAX_MBX_READ_ADDR    = 0x%04x", (unsigned int)MAX_MBX_READ_ADDRESS);
    ...
    /*
     * ---------------------------------------------------------
     * SM0: Receive Mailbox / Master Write
     * ---------------------------------------------------------
     */
    pSyncMan = GetSyncMan(MAILBOX_WRITE);
    ...
    SMLength = (UINT16)((pSyncMan->AddressLength & SM_LENGTH_MASK) >> SM_LENGTH_SHIFT);
    SMAddress = (UINT16)(pSyncMan->AddressLength & SM_ADDRESS_MASK);
    SMSettings = pSyncMan->Settings[0];

    if (!(SMSettings & SM_SETTING_ENABLE_VALUE))
    {
        DBG_APP("CHKSM SM0 FAIL: mailbox write SM not enabled, Settings=0x%08x",
                (unsigned int)SMSettings);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }
    else if ((SMSettings & SM_SETTING_DIRECTION_MASK) != SM_SETTING_DIRECTION_WRITE_VALUE)
    {
        DBG_APP("CHKSM SM0 FAIL: direction wrong, Settings=0x%08x ExpectedWrite=0x%02x",
                (unsigned int)SMSettings,
                (unsigned int)SM_SETTING_DIRECTION_WRITE_VALUE);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }
    else if ((SMSettings & SM_SETTING_MODE_MASK) != SM_SETTING_MODE_ONE_BUFFER_VALUE)
    {
        DBG_APP("CHKSM SM0 FAIL: mode wrong, Settings=0x%08x ExpectedOneBuffer=0x%02x",
                (unsigned int)SMSettings,
                (unsigned int)SM_SETTING_MODE_ONE_BUFFER_VALUE);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }
    else if (SMLength < MIN_MBX_SIZE)
    {
        DBG_APP("CHKSM SM0 FAIL: length too small, Len=0x%04x Min=0x%04x",
                (unsigned int)SMLength,
                (unsigned int)MIN_MBX_SIZE);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }
    else if (SMLength > MAX_MBX_SIZE)
    {
        DBG_APP("CHKSM SM0 FAIL: length too large, Len=0x%04x Max=0x%04x",
                (unsigned int)SMLength,
                (unsigned int)MAX_MBX_SIZE);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }
    else if (SMAddress < MIN_MBX_WRITE_ADDRESS)
    {
        DBG_APP("CHKSM SM0 FAIL: address too small, Addr=0x%04x Min=0x%04x",
                (unsigned int)SMAddress,
                (unsigned int)MIN_MBX_WRITE_ADDRESS);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }
    else if (SMAddress > MAX_MBX_WRITE_ADDRESS)
    {
        DBG_APP("CHKSM SM0 FAIL: address too large, Addr=0x%04x Max=0x%04x",
                (unsigned int)SMAddress,
                (unsigned int)MAX_MBX_WRITE_ADDRESS);
        result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
    }

    /*
     * ---------------------------------------------------------
     * SM1: Send Mailbox / Master Read
     * ---------------------------------------------------------
     */
    if (result == 0U)
    {
        ...
        if (!(SMSettings & SM_SETTING_ENABLE_VALUE))
        {
            DBG_APP("CHKSM SM1 FAIL: mailbox read SM not enabled, Settings=0x%08x",
                    (unsigned int)SMSettings);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
        else if ((SMSettings & SM_SETTING_DIRECTION_MASK) != SM_SETTING_DIRECTION_READ_VALUE)
        {
            DBG_APP("CHKSM SM1 FAIL: direction wrong, Settings=0x%08x ExpectedRead=0x%02x",
                    (unsigned int)SMSettings,
                    (unsigned int)SM_SETTING_DIRECTION_READ_VALUE);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
        else if ((SMSettings & SM_SETTING_MODE_MASK) != SM_SETTING_MODE_ONE_BUFFER_VALUE)
        {
            DBG_APP("CHKSM SM1 FAIL: mode wrong, Settings=0x%08x ExpectedOneBuffer=0x%02x",
                    (unsigned int)SMSettings,
                    (unsigned int)SM_SETTING_MODE_ONE_BUFFER_VALUE);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
        ...
        {
            DBG_APP("CHKSM SM1 FAIL: length too small, Len=0x%04x Min=0x%04x",
                    (unsigned int)SMLength,
                    (unsigned int)MIN_MBX_SIZE);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
        ...
        {
            DBG_APP("CHKSM SM1 FAIL: length too large, Len=0x%04x Max=0x%04x",
                    (unsigned int)SMLength,
                    (unsigned int)MAX_MBX_SIZE);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
        ...
        {
            DBG_APP("CHKSM SM1 FAIL: address too small, Addr=0x%04x Min=0x%04x",
                    (unsigned int)SMAddress,
                    (unsigned int)MIN_MBX_READ_ADDRESS);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
        else if (SMAddress > MAX_MBX_READ_ADDRESS)
        {
            DBG_APP("CHKSM SM1 FAIL: address too large, Addr=0x%04x Max=0x%04x",
                    (unsigned int)SMAddress,
                    (unsigned int)MAX_MBX_READ_ADDRESS);
            result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
        }
    }
    ...
    /*
     * ---------------------------------------------------------
     * SM3: Input Process Data / Master Read
     * ---------------------------------------------------------
     */
    if (result == 0U && maxChannel > PROCESS_DATA_IN)
    {
        b3BufferMode = TRUE;
        ...
        if ((SMSettings & SM_SETTING_ENABLE_VALUE) != 0U && SMLength == 0U)
        {
            DBG_APP("CHKSM IN FAIL: SM enabled but length is 0, Settings=0x%08x",
                    (unsigned int)SMSettings);
            result = SYNCMANCHSETTINGS + 1U;
        }
        else if (SMSettings & SM_SETTING_ENABLE_VALUE)
        {
            if (SMLength != nPdInputSize ||
                nPdInputSize == 0U ||
                SMLength > MAX_PD_INPUT_SIZE)
            {
                DBG_APP("CHKSM IN FAIL: input size mismatch, Addr=0x%04x Len=0x%04x Expected=0x%04x Max=0x%04x Settings=0x%08x",
                        (unsigned int)SMAddress,
                        (unsigned int)SMLength,
                        (unsigned int)nPdInputSize,
                        (unsigned int)MAX_PD_INPUT_SIZE,
                        (unsigned int)SMSettings);
                result = SYNCMANCHSIZE + 1U;
            }
            else
            {
                if ((SMSettings & SM_SETTING_DIRECTION_MASK) == SM_SETTING_DIRECTION_READ_VALUE)
                {
                    ...
                    {
                        if ((SMSettings & SM_SETTING_MODE_MASK) == SM_SETTING_MODE_ONE_BUFFER_VALUE)
                        {
                            b3BufferMode = FALSE;
                        }
                    }
                    ...
                    {
                        DBG_APP("CHKSM IN FAIL: input address invalid, Addr=0x%04x Allowed=0x%04x~0x%04x nEscAddrInputData=0x%04x nAlStatus=0x%04x",
                                (unsigned int)SMAddress,
                                (unsigned int)MIN_PD_READ_ADDRESS,
                                (unsigned int)MAX_PD_READ_ADDRESS,
                                (unsigned int)nEscAddrInputData,
                                (unsigned int)nAlStatus);
                        result = SYNCMANCHADDRESS + 1U;
                    }
                }
                else
                {
                    DBG_APP("CHKSM IN FAIL: input direction wrong, Settings=0x%08x ExpectedRead=0x%02x",
                            (unsigned int)SMSettings,
                            (unsigned int)SM_SETTING_DIRECTION_READ_VALUE);
                    result = SYNCMANCHSETTINGS + 1U;
                }
        ...
        else if (SMLength != 0U || nPdInputSize != 0U)
        {
            DBG_APP("CHKSM IN FAIL: SM disabled but size is non-zero, Len=0x%04x Expected=0x%04x Settings=0x%08x",
                    (unsigned int)SMLength,
                    (unsigned int)nPdInputSize,
                    (unsigned int)SMSettings);
            result = SYNCMANCHSIZE + 1U;
    ...
    /*
     * ---------------------------------------------------------
     * SM2: Output Process Data / Master Write
     * ---------------------------------------------------------
     */
    if (result == 0U && maxChannel > PROCESS_DATA_OUT)
    {
        pSyncMan = GetSyncMan(PROCESS_DATA_OUT);
        ...
        if ((SMSettings & SM_SETTING_ENABLE_VALUE) != 0U && SMLength == 0U)
        {
            DBG_APP("CHKSM OUT FAIL: SM enabled but length is 0, Settings=0x%08x",
                    (unsigned int)SMSettings);
            result = SYNCMANCHSETTINGS + 1U;
        }
        else if (SMSettings & SM_SETTING_ENABLE_VALUE)
        {
            if (SMLength == nPdOutputSize &&
                nPdOutputSize != 0U &&
                SMLength <= ((UINT16)MAX_PD_OUTPUT_SIZE))
            {
                if ((SMSettings & SM_SETTING_DIRECTION_MASK) == SM_SETTING_DIRECTION_WRITE_VALUE)
                {
                    if (((nAlStatus == STATE_PREOP) &&
                    ...
                    {
                        if (SMSettings & SM_SETTING_WATCHDOG_VALUE)
                        {
                            bWdTrigger = TRUE;
                        }
                        else
                    ...
                    {
                        DBG_APP("CHKSM OUT FAIL: output address invalid, Addr=0x%04x Allowed=0x%04x~0x%04x nEscAddrOutputData=0x%04x nAlStatus=0x%04x",
                                (unsigned int)SMAddress,
                                (unsigned int)MIN_PD_WRITE_ADDRESS,
                                (unsigned int)MAX_PD_WRITE_ADDRESS,
                                (unsigned int)nEscAddrOutputData,
                                (unsigned int)nAlStatus);
                        result = SYNCMANCHADDRESS + 1U;
                    }
                }
                ...
                {
                    DBG_APP("CHKSM OUT FAIL: output direction wrong, Settings=0x%08x ExpectedWrite=0x%02x",
                            (unsigned int)SMSettings,
                            (unsigned int)SM_SETTING_DIRECTION_WRITE_VALUE);
                    result = SYNCMANCHSETTINGS + 1U;
                }
            ...
            {
                DBG_APP("CHKSM OUT FAIL: output size mismatch, Addr=0x%04x Len=0x%04x Expected=0x%04x Max=0x%04x Settings=0x%08x",
                        (unsigned int)SMAddress,
                        (unsigned int)SMLength,
                        (unsigned int)nPdOutputSize,
                        (unsigned int)MAX_PD_OUTPUT_SIZE,
                        (unsigned int)SMSettings);
                result = SYNCMANCHSIZE + 1U;
            }
        }
        else if (SMLength != 0U || nPdOutputSize != 0U)
        {
            DBG_APP("CHKSM OUT FAIL: SM disabled but size is non-zero, Len=0x%04x Expected=0x%04x Settings=0x%08x",
                    (unsigned int)SMLength,
                    (unsigned int)nPdOutputSize,
                    (unsigned int)SMSettings);
            result = SYNCMANCHSIZE + 1U;
    ...
    /*
     * Acknowledge SM-change interrupt by reading enable byte of unused SM channels.
     */
    if (result == 0U)
    {
        for (i = maxChannel; i < nMaxSyncMan; i++)
