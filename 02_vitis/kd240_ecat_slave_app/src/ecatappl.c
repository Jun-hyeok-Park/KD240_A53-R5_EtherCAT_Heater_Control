...
#define SKIP_SYSTEMTIME_READ_FAILURE_TEST   1
/*-----------------------------------------------------------------------------------------
------
...
UINT16 MainInit(void)
{
    ...
    DBG_BOOT("MainInit: before SET_EEPROM_PTR");
    SET_EEPROM_PTR
    DBG_BOOT("MainInit: after SET_EEPROM_PTR");
#endif
    ...
    u32SystemTimeReadFailure = 0;
    ...
    #if (SKIP_SYSTEMTIME_READ_FAILURE_TEST == 0)
    
        {
            ...
            while (u32Cnt < 1000U)
            {
                ...
                {
                    u32Delta = u32Delta + (0xFFFFFFFFU - u32TimeValue);
                }

                if (u32SystemTimeReadFailure == 0U)
                {
        ...
        DBG_BOOT("MainInit: SystemTime read test skipped");
        u32SystemTimeReadFailure = 0U;
